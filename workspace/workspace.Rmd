...
```{r join-US-states-geometry-to-CAIDI}
...
```{r filter-MajorEventDays-FALSE}
CAIDI_leaflet_df <- CAIDI_leaflet_df %>%
  filter(`Major Event Days` == FALSE)
```

```{r leaflet-CAIDI-bins&palette}
CAIDI_leaflet_df %>%
  filter(Year == "2022") %>%
  summary("CAIDI")
  
bins <- seq(from = 75, to = 225, by = 75)
#yblu <- c("#ffffd9", "#edf8b1", "#c7e9b4","#41b6c4", "#1d91c0", "#0c2c84","gray", "black" )
palCAIDI <- colorBin("PiYG", domain = CAIDI_leaflet_df$CAIDI, bins = bins)

labels <- sprintf("<strong>%s</strong><br/>%g", 
                  CAIDI_leaflet_df$`Census Division\r\nand State`, CAIDI_leaflet_df$CAIDI) %>% lapply(htmltools::HTML)
```

```{r leaflet-CAIDI-2022, eval=FALSE}
CAIDI_leaflet_df |>
  filter(Year == "2022") |>
  #filter(`Census Division\r\nand State` %in% c("Alabama", "Alaska", "Arizona", "Arkansas", "California", "Colorado", "Connecticut", "Delaware", "Florida", "Georgia", "Hawaii", "Idaho", "Illinois", "Indiana", "Iowa", "Kansas", "Kentucky", "Louisiana", "Maine", "Maryland", "Massachusetts", "Michigan", "Minnesota", "Mississippi", "Missouri", "Montana", "Nebraska", "Nevada", "New Hampshire", "New Jersey", "New Mexico", "New York", "North Carolina", "North Dakota", "Ohio", "Oklahoma", "Oregon", "Pennsylvania", "Rhode Island", "South Carolina",  "South Dakota", "Tennessee", "Texas", "Utah", "Vermont", "Virginia", "Washington", "West Virginia", "Wisconsin", "Wyoming", "Illinois", "Montana", "District of Columbia")) |>
leaflet() |>
  addTiles() |>
  setView(lng = -98.6,
          lat = 36.7,
          zoom = 4) |>
  addPolygons(
    fillColor = ~palCAIDI(CAIDI_leaflet_df$CAIDI), 
    color = "white", 
    weight = 1,
    label = labels
)
