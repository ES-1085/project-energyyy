...
disturbance <- 
  ...
  inner_join(energy_sector, by = "Entity")
...
disturbance_select <- disturbance |>
  select(`Revenues (Thousands Dollars)`, Ownership, Entity) 
...
disturbance_select |>
  group_by(`Revenues (Thousands Dollars)` ) |>
  count()
...
disturbance_count <- 
  ...
  group_by(Entity) |>
  count()
  inner_join(disturbance_select)
```

```{r}
disturbance_count <- disturbance_count|>
  
  group_by(Entity, Ownership, n) |>
  summarise(sum_revenue = sum(`Revenues (Thousands Dollars)`)) |>
  as.data.frame()
...
ggplot(disturbance, aes(
  x = `Revenues (Thousands Dollars)`, 
  y = `Ownership`,
  size = `Entity`)) +
  geom_point()
  #geom_point(data = disturbance_count, aes(size = disturbance_count$n))
```
